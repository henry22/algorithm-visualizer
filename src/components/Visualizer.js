...
export default function Visualizer() {
	...
	function submit() {
		...
		for (let i = 0; i < customNumbers.length; i++) {
			...
			customItems.push({ id: uid(Math.random()), itemValue: Number(value), color: '#2d8ae2', IsBeingSwapped: false })
		}
			...
			<Container maxWidth="lg" style={{ height: '100vh' }}>
				<Grid container spacing={3}>
					...
					<Grid item xs={12} style={{ display: 'flex', justifyContent: 'space-between' }}>
						<Button disabled={process} variant="contained" color="primary" onClick={() => runAlgorithm()}>Sort!</Button>
						<Button color="secondary" variant="contained" onClick={resetNumbers}>RESET</Button>
				...
				<Snackbar
					...
					action={
						<React.Fragment>
							<Button color="secondary" size="small" onClick={resetNumbers}>RESET</Button>
						</React.Fragment>
					}
				/>
			</Container>
